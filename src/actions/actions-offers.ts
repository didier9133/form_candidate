...
const proposalArray = [
  ...
  {
    value: "01HYPWTREKWRSJ092E7B521QTJ",
    label: "Líder Tecnico Javascript",
  },
];
const sleep = (ms: number) => new Promise(resolve => setTimeout(resolve, ms));

export async function getCountriesWithCode() {
  try {
    await sleep(300);
...
export async function getProposalJob() {
  try {
    await sleep(300);
...
export async function getCountryDefault() {
  try {
    await sleep(400);
    return {
      code: "CO",
...
export async function isSuccessFormRegister(id: string) {
  try {
    await sleep(200);
    if (id !== "123") throw new Error("Error");
    return {
      success: true,
      data: {
        name: "Juan",
